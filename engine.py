...
async def engine(board):
    ...
    resp_string = f"{choice.move} {round(choice.points, 3)} {choice.depth} --> "
    best_response = None
    ...
    if choice.responses:
        ...
        if best_response:
            resp_string += f"{best_response.move} {round(best_response.points, 3)} {best_response.depth}"
    while best_response and len(best_response.responses) > 0:
            best_response = min(best_response.responses, key=lambda x: -1 * x.points, default=None)
            resp_string += f"--> {best_response.move} {round(best_response.points, 3)} {best_response.depth}"
    print(resp_string)
    ...
    # if best_response:
    #     penultimate_move = best_response.parent
    # print("best, penultimate", best_response.move, penultimate_move.move)
    ...
    if choice.responses:
        ...
        if best_response:
            resp_string += f"{best_response.move} {round(best_response.points, 3)} {best_response.depth}"
            diff = best_response.parent.diff()
    while best_response is not None and len(best_response.responses) > 0:
       best_response = min(best_response.responses, key=lambda x: -1 * x.points, default=None)
       resp_string += f"--> {best_response.move} {round(best_response.points, 3)} {best_response.depth}"
       diff = best_response.parent.diff()
    
    # if best_response:
    #     for resp in best_response.parent.responses:
    #         print(resp.move, resp.points)
    #     for weight in best_response.parent.weights:
    #         print(weight)
